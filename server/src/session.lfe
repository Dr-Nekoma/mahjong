  ...
  (receive
    ('player-connect
     (let* ((players (map-get state 'players))
	    (last-player-id (erlang:length players)))
       (if (< last-player-id 4)
	 (room (collections:update-in state '(players)
			 (cons (tuple (+ last-player-id 1)
